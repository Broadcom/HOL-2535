# vSphere.py version 1.11 11-June 2024
# custom version for 2535 to check and repair NFS datastore
import datetime
import os
import sys
...
# special for 2535 - repair NFS datastores
lsf.write_output('Checking NFS datastore...')
lsf.connect_vc('esx-01a.vcf.sddc.lab', 'root', lsf.password)
host = lsf.get_host('esx-01a.vcf.sddc.lab')
ds = lsf.get_datastore('ds-nfs01')
if not ds.summary.accessible:
    lsf.write_output('Repairing NFS datastore...')   
    ds_system = host.configManager.datastoreSystem
    ds_system.RemoveDatastore(ds)
    lsf.labstartup_sleep(5)
    ds = ds_system.CreateNasDatastore(
    vim.host.NasVolume.Specification(remoteHost='10.10.20.60',
                                     remotePath='/mnt/NFS01',
                                     localPath='ds-nfs01',
